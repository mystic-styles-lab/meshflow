...
def create_user(db: Session, user: UserCreate, admin: Admin = None) -> User:
    ...
    # Apply tariff settings if tariff_id is provided
    data_limit = user.data_limit
    expire = user.expire
    tariff_id = getattr(user, 'tariff_id', None)
    
    if tariff_id:
        from app.services.tariffs.database import TariffsDatabase
        tariffs_db = TariffsDatabase()
        tariff = tariffs_db.get_tariff(tariff_id)
        
        if tariff:
            # Set data limit from tariff (0 for unlimited traffic)
            if tariff.get('unlimited_traffic', 0) == 1:
                data_limit = 0
            else:
                data_limit = tariff.get('traffic_limit', 0)
            
            # Calculate expiry date: current timestamp + duration_days
            if tariff.get('duration_days', 0) > 0:
                from time import time
                expire = int(time()) + (tariff['duration_days'] * 86400)
...
def update_user(db: Session, dbuser: User, modify: UserModify) -> User:
    ...
    # Update tariff_id if provided (including None to clear it)
    if hasattr(modify, 'tariff_id'):
        old_tariff_id = dbuser.tariff_id
        dbuser.tariff_id = modify.tariff_id
        
        # Apply tariff settings when tariff changes
        if modify.tariff_id and modify.tariff_id != old_tariff_id:
            from app.services.tariffs.database import TariffsDatabase
            from time import time
            tariffs_db = TariffsDatabase()
            tariff = tariffs_db.get_tariff(modify.tariff_id)
            
            print(f"[TARIFF DEBUG] User: {dbuser.username}, Old tariff: {old_tariff_id}, New tariff: {modify.tariff_id}")
            print(f"[TARIFF DEBUG] Tariff data: {tariff}")
            
            if tariff:
                # Set data limit from tariff (0 for unlimited traffic)
                if tariff.get('unlimited_traffic', 0) == 1:
                    dbuser.data_limit = 0
                    print(f"[TARIFF DEBUG] Set unlimited data_limit = 0")
                else:
                    # Always apply traffic limit from tariff when tariff changes
                    dbuser.data_limit = tariff.get('traffic_limit', 0) or None
                    print(f"[TARIFF DEBUG] Set data_limit = {dbuser.data_limit}")
                
                # Always calculate and set expiry date when tariff changes
                if tariff.get('duration_days', 0) > 0:
                    new_expire = int(time()) + (tariff['duration_days'] * 86400)
                    dbuser.expire = new_expire
                    print(f"[TARIFF DEBUG] Set expire = {new_expire} (duration_days: {tariff['duration_days']})")
                else:
                    print(f"[TARIFF DEBUG] No duration_days in tariff: {tariff.get('duration_days')}")
            else:
                print(f"[TARIFF DEBUG] Tariff not found in database!")
