...
class XRayCore:
    def __init__(self,
        ...
        self.mock_mode = not os.path.exists(self.executable_path)

        if self.mock_mode:
            logger.warning(f"Xray executable not found at {executable_path}. Running in MOCK mode.")
            self.version = "0.0.0-mock"
        else:
    ...
    def start(self, config: XRayConfig):
        ...
        if self.mock_mode:
            logger.warning("Xray start skipped (MOCK mode)")
    ...
    def stop(self):
        ...
        if self.mock_mode:
            self.process = None
            logger.warning("Xray core stopped (MOCK mode)")
        else:
