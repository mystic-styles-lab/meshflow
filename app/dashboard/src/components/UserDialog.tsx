...
const baseSchema = {
  ...
  tariff_id: z.union([
    z.number().min(1, { message: "Выберите тариф" }),
    z.null()
  ]).optional(),
...
export const UserDialog: FC<UserDialogProps> = () => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      fetch("/tariffs/?enabled_only=true")
  ...
  const submit = (values: FormType) => {
    // Валидация: тариф обязателен при создании
    if (!isEditing && (!values.tariff_id || values.tariff_id < 1)) {
      form.setError("tariff_id", {
        type: "manual",
      ...
      <FormProvider {...form}>
        ...
        >
          <form onSubmit={form.handleSubmit(submit)}>
                    ...
                    >
                      ...
                      <FormControl mb={"10px"}>
                        ...
                        <FormHelperText fontSize="xs" color="gray.500">
                          ID пользователя в Telegram (необязательно)
                        </FormHelperText>
                      </FormControl>
                      <Flex gap={3} mb={"10px"} alignItems="flex-end">
                        <FormControl flex="1">
                          ...
                          <Controller
                            ...
                            name="tariff_id"
                            render={({ field }) => {
                              const selectedTariff = tariffs.find(t => t.id === field.value);
                              return (
                                <Menu>
                                  <MenuButton
                                    as={Button}
                                    ...
                                    isDisabled={disabled || tariffsLoading}
                                  ...
                                  <Portal>
                                    ...
                                    >
                                      ...
                                      {tariffs.map((tariff) => (
                                        <MenuItem
                                          key={tariff.id}
                                          bg={field.value === tariff.id ? "linear-gradient(135deg, rgba(102, 126, 234, 0.3), rgba(128, 90, 213, 0.3))" : "transparent"}
                                          color="white"
                                          _hover={{
                                            bg: "linear-gradient(135deg, rgba(102, 126, 234, 0.3), rgba(128, 90, 213, 0.3))",
                                          }}
                                          _light={{
                                            color: "gray.800",
                                            bg: field.value === tariff.id ? "linear-gradient(135deg, rgba(102, 126, 234, 0.2), rgba(128, 90, 213, 0.2))" : "transparent",
                                            _hover: {
                                              bg: "linear-gradient(135deg, rgba(102, 126, 234, 0.15), rgba(128, 90, 213, 0.15))",
                                            },
                                          }}
                                          borderRadius="8px"
                                          mx={2}
                                          onClick={() => field.onChange(tariff.id)}
                                        >
                                          {tariff.name} - {tariff.price} ₽ ({tariff.durationDays} дн., {tariff.unlimitedTraffic ? '∞' : `${(tariff.trafficLimit / 1073741824).toFixed(0)} GB`})
                                        </MenuItem>
                                      ))}
                                    </MenuList>
                                  </Portal>
                          ...
                          {form.formState.errors.tariff_id && (
                            <FormErrorMessage>
                              {form.formState.errors.tariff_id.message}
                            </FormErrorMessage>
                          )}
                        </FormControl>
                        {!isEditing && (
            ...
            <ModalFooter mt="3" borderTop="1px solid rgba(255, 255, 255, 0.08)" _light={{ borderTop: "1px solid rgba(0, 0, 0, 0.08)" }}>
              <HStack
              ...
              >
                    ...
                    <>
                      <Tooltip label={t("delete")} placement="top">
                        <IconButton
                          ...
                          color="red.400"
                          boxShadow="0 2px 8px rgba(0, 0, 0, 0.15), inset 0 1px 0 rgba(255, 255, 255, 0.1)"
                          _light={{ bg: "rgba(255, 255, 255, 0.8)", border: "1px solid rgba(0, 0, 0, 0.1)", color: "red.500", boxShadow: "0 2px 8px rgba(0, 0, 0, 0.08), inset 0 1px 0 rgba(255, 255, 255, 0.5)" }}
                          _hover={{ bg: "rgba(255, 255, 255, 0.1)", _light: { bg: "rgba(255, 255, 255, 0.95)" } }}
                          onClick={() => {
                ...
                >
                  <Box
                    ...
                    _hover={disabled ? {} : { bg: "rgba(102, 126, 234, 0.9)", boxShadow: "0 6px 20px rgba(102, 126, 234, 0.5), inset 0 1px 0 rgba(255, 255, 255, 0.3)" }}
                    display="flex"
                    alignItems="center"
                    gap={2}
                  >
